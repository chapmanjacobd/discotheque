...
test.describe('Categorization Workflow - Full Process', () => {
  ...
  test('completes full categorization workflow: create category, add keywords, run categorization', async ({ page, server }) => {
    ...
    console.log(`Categories after adding defaults: ${categoryCount}`);
    ...
    // Handle prompt for category name
    ...
    // Step 5: Find potential keywords
    ...
    if (await findKeywordsBtn.count() > 0) {
      ...
      if (keywordText) {
        ...
        const keywordInCategory = customCategoryCard.locator(`.curation-tag[data-keyword="${keywordText}"]`);
  ...
  test('adds keyword to existing category', async ({ page, server }) => {
    ...
    if (await findKeywordsBtn.count() > 0) {
      ...
      if (suggestionCount > 0) {
        ...
        if (keywordText) {
          // Add keyword to first category
          const categories = page.locator('.curation-cat-card');
          const firstCategory = await categories.first().getAttribute('data-category');
          
          page.on('dialog', async dialog => {
            if (dialog.type() === 'prompt' && firstCategory) {
              await dialog.accept(firstCategory);
            }
          });
          await firstSuggestion.click();
          await page.waitForTimeout(1000);
          ...
          const categoryCard = page.locator(`.curation-cat-card[data-category="${firstCategory}"]`);
